...
# 720000
# blocks = rpc_connection.batch_([ [ "getblock", '00000000003294277d8efe6a48ea0dc7dba64aa3829e5d416edd6abc8dd8a3af', 2 ] ])
# print(blocks)
    ...
    for tx in txs:
      ...
      for t_in in tx['vin']:
        if 'txid' not in t_in:
          ...
          # print(t_in['txid'])
      # if 'fee' in tx.keys():
      #   print(tx['txid'], tx['fee'])
      # else:
      #   print(tx['txid'], ' - No fees')
      # t_ins = tx['vin']
      # for t_in in t_ins:
      #   print(t_ins)
      # {'value': Decimal('50.00000000'), 'n': 0
      # , 'scriptPubKey': {'asm': '04e6565c3c615b6f5d1033c6d32c3ad0307794a0cc7e551c792bec6f9bfe481777e47ac745136798663b8ddb57a76744106293453b89dad1e411485281d28b7a08 OP_CHECKSIG'
      # , 'hex': '4104e6565c3c615b6f5d1033c6d32c3ad0307794a0cc7e551c792bec6f9bfe481777e47ac745136798663b8ddb57a76744106293453b89dad1e411485281d28b7a08ac'
      # , 'type': 'pubkey'}}
          # if 'fee' in t_out.keys():
          #   print(tx['txid'], str(t_out['fees']))
          # else:
          #   print(tx['txid'], ' - No fees')
          #print(re.sub("[^0-9]", "", t_out['value'].astype('str')))

    #transactions_out = (tx_id, t_out['n'], t_out['value'], t_out['fee'], t_out['scriptPubKey'], t_out['scriptPubKey']['address'])
